...
json_list = ['Llama-2-7b-chat-hf', 'Llama-2-13b-chat-hf', 'Meta-Llama-3-8B-Instruct', 'c4ai-command-r-v01', 'gpt-4o-mini-2024-07-18',\
...
parser.add_argument('-e', '--epochs', type=int, default=20, help='batch size')
parser.add_argument('--cuda', '-c', type=str, default='0', help='gpu ids, like: 1,2,3')
parser.add_argument('--seed', '-s', type=int, default=42, help='random seed.')
parser.add_argument('--max-length', type=int, default=512, help='max_length')
parser.add_argument("--dataset", default='hc3')
parser.add_argument("--all-train", action="store_true", default=False, help='use all data for training')
...
def main(args: argparse.Namespace):
        ...
        with open(file_name) as f:
            ...
            for i in range(args.data_size):
                ...
                if len(tokenized_response) < 100:
                    continue
                train_list.append((tokenized_response[:args.max_length], json_list.index(model_id)))
    ...
    for model_id in json_list:
        ...
        with open(file_name) as f:
            ...
            for i in range(args.data_size, args.data_size+args.eval_size):
                ...
                if len(tokenized_response) < 100:
                    continue
                test_list.append((tokenized_response[:args.max_length], json_list.index(model_id)))
    ...
    model = AutoModelForSequenceClassification.from_pretrained(args.model_name, num_labels=len(json_list), device_map='auto')
